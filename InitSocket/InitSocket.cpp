...
int main(int argc, char* argv[])
{
	// 윈속 초기화
	WSADATA wsa;
	if (WSAStartup(((WORD)LOBYTE(2) | ((WORD)LOBYTE(2) << 8)), &wsa) != 0)
		return 1;
	printf("%d.%d, %d.%d, %s, %s\n", LOBYTE(wsa.wVersion), HIBYTE(wsa.wVersion), LOBYTE(wsa.wHighVersion), HIBYTE(wsa.wHighVersion), 
		wsa.szDescription, wsa.szSystemStatus);
	printf("[알림] 윈속 초기화 성공\n");
	// 소켓 생성
